 ...
 * @since : 2021/11/3 14:37
 **/
@Slf4j
public class QuotNettyTcpClient extends NettyTcpClient {
	...
	public QuotNettyTcpClient(String serviceNm, String host, int port, List<String> instrumentList) {
	...
	@Override
	public void connect() {
		//log.info("NettyTcpClient is Reconnect by NettyTcpClient.connect");
		try {
		...
		SubscribeInstrumentReqVO req = new SubscribeInstrumentReqVO();
		try {
			for (String instrument : instrumentList) {
				req.setSize(36);
				req.setCmdId(101);
				req.setRequestId(REQUEST_ID++);
				req.setInstrumentId(instrument);
				sendMsg(req.toByteBuf());
			}
		} catch (Exception e) {
			StringBuilder sb = new StringBuilder();
			sb.append("发送Netty消息失败。");
			sb.append("ServiceNm：" + serviceNm);
			sb.append("host:" + host);
			sb.append("port:" + port);
