...
import com.xinqi.ctp_puppet.netty.gateway.tcp.vo.CTPReqVOBase;
import com.xinqi.ctp_puppet.netty.gateway.tcp.vo.CTPRespVOBase;
...
import com.xinqi.ctp_puppet.ws.WSClient;
...
public class TradeTcpServerFactory {
	...
	private static List<CTPRespVOBase> RTN_HISTORY = new CopyOnWriteArrayList<>();
	...
	public static void addRtnHistory(CTPRespVOBase byteBuf) {
		RTN_HISTORY.add(byteBuf);
	}
	public static void notifyRtnHistory(Channel channel) {
		for (CTPRespVOBase vo : RTN_HISTORY) {
			CTPOnFuncMsgHandleEnum handleEnum = CTPOnFuncMsgHandleEnum.getEnum(vo);
			if (null != handleEnum) {
